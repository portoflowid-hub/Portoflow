...
import styles from '../styles/login/AuthForm.module.css'; // Path styling disesuaikan
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await fetch('http://localhost:5000/api/register', {
        ...
        // 2. Kirim semua data yang dibutuhkan backend
        body: JSON.stringify({ 
          fullName, 
          username, 
          email, 
      ...
      <div className={styles.pageWrapper}>
        <div className={styles.formContainer}>
          ...
          <form className={styles.form} onSubmit={handleSubmit}>
            {/* 3. Tambahkan elemen form untuk field baru */}
            <div className={styles.inputGroup}>
              <label htmlFor="fullName" className={styles.label}>Nama Lengkap</label>
              <input type="text" id="fullName" value={fullName} onChange={(e) => setFullName(e.target.value)} className={styles.input} required />
            </div>
            ...
            <div className={styles.inputGroup}>
              ...
              <input type="date" id="dateOfBirth" value={dateOfBirth} onChange={(e) => setDateOfBirth(e.target.value)} className={styles.input} required />
            </div>

            <div className={styles.inputGroup}>
                <label htmlFor="gender" className={styles.label}>Jenis Kelamin</label>
                <select id="gender" value={gender} onChange={(e) => setGender(e.target.value)} className={styles.input} required>
          ...
          <button type="button" className={styles.googleButton}><FcGoogle size={22} /> Daftar dengan Google</button>
          <p className={styles.redirectText}>Sudah punya akun? <Link href="/login">Masuk sekarang</Link></p>
        </div>
      </div>
    </>
  );
