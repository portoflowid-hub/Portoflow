...
const validateUserIds = async (ids = []) => {
  ...
  const found = await User.find({ _id: { $in: filtered } }).select('_id').lean();
...
export const getAllCourses = async (req, res) => {
  try {
    ...
    if (instructor && mongoose.isValidObjectId(instructor)) filter.instructors = instructor;
    ...
    const counts = await Enrollment.aggregate([
      { $match: { course: { $in: courseIds }, role: 'student', status: 'enrolled' } },
      { $group: { _id: '$course', count: { $sum: 1 } } }
    ]);

    const countMap = counts.reduce((m, r) => { m[r._id.toString()] = r.count; return m; }, {});
    ...
    if (req.user?.id) {
      ...
      userEnrolledMap = userEnrolls.reduce((m, e) => { m[e.course.toString()] = true; return m; }, {});
    }
...
export const getCourseById = async (req, res) => {
  try {
    const { id } = req.params;
    if (!mongoose.isValidObjectId(id)) return res.status(400).json({ status: 'fail', message: 'Invalid course id' });
    ...
    const studentCount = await Enrollment.countDocuments({ course: id, role: 'student', status: 'enrolled' });

    const isEnrolled = req.user?.id
      ? !!(await Enrollment.findOne({ course: id, user: req.user.id, status: 'enrolled' }).select('_id'))
      : false;

    res.status(200).json({ status: 'success', data: { ...course, studentCount, isEnrolled } });
  } catch (err) {
    res.status(500).json({ status: 'error', message: err.message });
  }
};
...
export const createCourse = async (req, res) => {
  ...
  try {
    const { title, description = '', price = 0, instructors = [], teachingAssistants = [], capacity, tags = [], category = '', level = 'beginner', language, imageUrl, isPublished } = req.body;
    ...
    const validInstructors = await validateUserIds(Array.isArray(instructors) ? instructors : []);
    const validTAs = await validateUserIds(Array.isArray(teachingAssistants) ? teachingAssistants : []);

    // create course
    const course = await Course.create([{
      title, description, price, createdBy: req.user.id,
      instructors: validInstructors, teachingAssistants: validTAs,
      capacity: capacity ?? null, tags, category, level, language, imageUrl, isPublished: !!isPublished
    }], { session });

    // create Enrollment records for instructors & TAs (so they appear in enrollment lists)
    const enrollDocs = [];
    validInstructors.forEach(uid => enrollDocs.push({ user: uid, course: course[0]._id, role: 'instructor', status: 'enrolled' }));
    validTAs.forEach(uid => enrollDocs.push({ user: uid, course: course[0]._id, role: 'ta', status: 'enrolled' }));
...
export const updateCourse = async (req, res) => {
  ...
  try {
    ...
    if (updates.instructors) {
      const valid = await validateUserIds(Array.isArray(updates.instructors) ? updates.instructors : []);
      updates.instructors = valid;
    }
    if (updates.teachingAssistants) {
      const valid = await validateUserIds(Array.isArray(updates.teachingAssistants) ? updates.teachingAssistants : []);
    ...
    if (typeof updates.capacity !== 'undefined' && updates.capacity !== null) {
      const currentEnrolled = await Enrollment.countDocuments({ course: id, role: 'student', status: 'enrolled' }).session(session);
      if (updates.capacity < currentEnrolled) {
        await session.abortTransaction();
        return res.status(400).json({ status: 'fail', message: `Capacity cannot be less than current enrolled (${currentEnrolled})` });
      }
    }

    const course = await Course.findByIdAndUpdate(id, updates, { new: true, runValidators: true, session })
      .populate('createdBy', 'username email')
      .populate('instructors', 'fullName username email')
      .populate('teachingAssistants', 'fullName username email');
      ...
      updates.instructors.forEach(uid => {
        enrollOps.push({
          updateOne: {
            ...
            update: { $setOnInsert: { user: uid, course: course._id, role: 'instructor', status: 'enrolled', enrolledAt: new Date() } },
            upsert: true
          }
        });
      });
      ...
      updates.teachingAssistants.forEach(uid => {
        enrollOps.push({
          updateOne: {
            ...
            update: { $setOnInsert: { user: uid, course: course._id, role: 'ta', status: 'enrolled', enrolledAt: new Date() } },
            upsert: true
          }
        });
      });
...
export const listCourseStudents = async (req, res) => {
  try {
    ...
    res.status(200).json({ status: 'success', data: rows, meta: { total, page: Number(page), limit: Number(limit) } });
  } catch (err) {
    res.status(500).json({ status: 'error', message: err.message });
  }
};
